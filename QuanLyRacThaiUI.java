...
//        mainPanel.add(createNvdpPanel(), "Quản lý nhân viên điều phối");
        ...
        mainPanel.add(createThongKePhanCongPanel(), "Quản lý thống kê phân công");
...
// ... existing code ...
   ...
   private void showAddLichThuGomDialog(DefaultTableModel model) {
    ...
    for (int h = 7; h <= 23; h++) {
        ...
        if (h != 23) gioThuList.add(String.format("%d:30", h));
    }
...
formPanel.add(maNvdpField);
      ...
      formPanel.add(new JLabel("Trạng thái:"));
String[] trangThaiOptions = {"Chưa xác nhận", "Đã xác nhận"};
JComboBox<String> trangThaiCombo = new JComboBox<>(trangThaiOptions);
formPanel.add(trangThaiCombo);
...
inputPanel.add(trangThaiCombo, "combo");
    ...
    private JPanel createThongKePanel() {
        ...
        // Panel 1: Thống kê khối lượng rác (Pie Chart)
        ...
        ChartPanel wasteChartPanel = new ChartPanel(wasteChart);
        wasteChartPanel.setBackground(Color.WHITE);
        
        JPanel weightPanel = new JPanel(new BorderLayout());
        weightPanel.setBackground(Color.WHITE);
        weightPanel.add(wasteChartPanel);
        ...
        try {
            ...
            try (PreparedStatement pstmt = conn.prepareStatement(revenueQuery)) {
                ...
                ResultSet rs = pstmt.executeQuery();
                if (rs.next()) {
                    revenueDataset.addValue(rs.getDouble("DoanhThu"), "Doanh thu", "Tháng " + currentMonth);
                }
            }
        } catch (SQLException ex) {
            ex.printStackTrace();
    ...
    private void updateAllCharts() {
        try {
            ...
            // 1. Update waste volume pie chart
            DefaultPieDataset wasteDataset = new DefaultPieDataset();
            String wasteQuery = "SELECT dv.TenDichVu, SUM(cthd.KhoiLuong) as TongKL " +
                               "FROM ChiTietHopDong cthd " +
                               "JOIN DichVu dv ON cthd.MaDichVu = dv.MaDichVu " +
            ...
            // 4. Update service request status pie chart
            DefaultPieDataset requestDataset = new DefaultPieDataset();
            String requestQuery = "SELECT TrangThai, COUNT(*) as SoLuong " +
                                 "FROM YeuCauDatLich " +
                                 "WHERE EXTRACT(MONTH FROM ThoiGianYc) = ? " +
                                 "GROUP BY TrangThai";
            try (PreparedStatement pstmt = conn.prepareStatement(requestQuery)) {
                ...
                ResultSet rs = pstmt.executeQuery();
                while (rs.next()) {
                    requestDataset.setValue(rs.getString("TrangThai"), rs.getInt("SoLuong"));
                }
            }
            
            // Update the charts in the UI
            JPanel contentPanel = (JPanel) ((JPanel) mainPanel.getComponent(mainPanel.getComponentCount() - 1)).getComponent(1);
            
            // Update waste chart
            JPanel weightPanel = (JPanel) contentPanel.getComponent(0);
            ChartPanel wasteChartPanel = (ChartPanel) weightPanel.getComponent(0);
            PiePlot wastePlot = (PiePlot) wasteChartPanel.getChart().getPlot();
            wastePlot.setDataset(wasteDataset);
            
            // Update revenue chart
            JPanel revenuePanel = (JPanel) contentPanel.getComponent(1);
            ChartPanel revenueChartPanel = (ChartPanel) revenuePanel.getComponent(0);
            CategoryPlot revenuePlot = (CategoryPlot) revenueChartPanel.getChart().getPlot();
            revenuePlot.setDataset(revenueDataset);
            
            // Update staff chart
            JPanel staffPanel = (JPanel) contentPanel.getComponent(2);
            ChartPanel staffChartPanel = (ChartPanel) staffPanel.getComponent(0);
            CategoryPlot staffPlot = (CategoryPlot) staffChartPanel.getChart().getPlot();
            staffPlot.setDataset(staffDataset);
            
            // Update request chart
            JPanel requestPanel = (JPanel) contentPanel.getComponent(3);
            ChartPanel requestChartPanel = (ChartPanel) requestPanel.getComponent(0);
            PiePlot requestPlot = (PiePlot) requestChartPanel.getChart().getPlot();
            requestPlot.setDataset(requestDataset);
